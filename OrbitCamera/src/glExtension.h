...
// NOTE: The size of HDC in 64bit Windows is 8 bytes, so the param of this
// class is "long" instead of "int".
//
...
public:
    ~glExtension();
    static glExtension& getInstance(long param=0);          // must be called after RC is open
...
private:
    glExtension(long param=0);                              // prevent calling ctor
    glExtension(const glExtension& rhs);                    // no implementation
    void getExtensionStrings(long param=0);
